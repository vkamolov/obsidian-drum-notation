...
In the web playground, **Focus view** toggles a spacious notation workspace by
hiding setup, source and editing panels. Use the same button to restore them.
It remains available during playback, but is unavailable during visual editing
or verification. The choice lasts for the current page session, including cached
Back navigation; reloading restores the ordinary layout. Obsidian keeps its normal
block layout and has no Focus toggle.

Tempo shows effective BPM in the playground; Obsidian also shows the selected
percentage when using a speed preset. Loop, Tempo and Metronome retain their
practice tools in both hosts.
...
`drum-notation-importer` Agent Plugin. Version 0.2.1 transcribes clean printed
...
1.6.0 notation core, and reports ambiguities and format workarounds. Audio
...
`.tar.gz` packages from it: a strict Agent Plugins 1.0.0 package plus
compatibility packages for Codex/OpenAI, Claude Code, and Gemini CLI. Release
0.2.1 also includes a separate skills-only OpenAI directory ZIP. The client
adapters only provide packaging metadata; transcription instructions,
references, and the network-free validator remain single-source inside
`skills/import-drum-score/`.

Importer releases use tags such as `agent-plugin-v0.2.1`; they do not change
the Obsidian plugin version or release assets.

See the [importer website](https://vkamolov.github.io/obsidian-drum-notation/importer/)
for scope, privacy, terms, and support. The 0.2.1 package is prepared for public
ChatGPT/Codex directory review; GitHub marketplace installation remains
available independently while review is pending.
...
[`agent-plugin-v0.2.1` release](https://github.com/vkamolov/obsidian-drum-notation/releases/tag/agent-plugin-v0.2.1),
then install the extracted directory:

```bash
gh release download agent-plugin-v0.2.1 \
  --repo vkamolov/obsidian-drum-notation \
  --pattern "drum-notation-importer-0.2.1-gemini.tar.gz"
tar -xzf drum-notation-importer-0.2.1-gemini.tar.gz
